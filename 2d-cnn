...
input_data_path = 'data_file\\use_data\\(normalization, exist gait data)input_data.xlsx'
label_data_path = 'data_file\\use_data\\Stride_Length_Data.xlsx'
def get_csv():
    input_df = pd.read_excel(input_data_path, skiprows = 0, engine='openpyxl')  #앞의 1행 생략
    label_df = pd.read_excel(label_data_path, skiprows = 0, engine='openpyxl')  #앞의 1행 생략
...
if __name__ == '__main__':
    ...
    gait_data = gait_data.reshape(-1,137,1)
    ...
    model.add(Conv1D(filters=1024, kernel_size=2,padding='same', activation='relu', input_shape=(137,1)))
    ...
    model.add(MaxPooling1D(2))
    model.add(Dense(1024, activation='relu', kernel_regularizer=reg.L2(0.05)))
