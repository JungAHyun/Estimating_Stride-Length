...
def save_dataset(x_train, x_test, y_train, y_test):
    ...
    for i in range(112):
        tmp = []
        for j in range(375):
            tmp.append(x_test[i][j][0])
        x_test_list.append(tmp)


    x_train_dataset = pd.DataFrame(data=x_train_list)
    x_train_dataset.to_csv('x_train_dataset.csv')

    x_test_dataset = pd.DataFrame(data=x_test_list)
    x_test_dataset.to_csv('x_test_dataset.csv')
    ...
    y_train_dataset = pd.DataFrame(data=y_train_list)
    y_train_dataset.to_csv('y_train_dataset.csv')

    y_test_dataset = pd.DataFrame(data=y_test_list)
    y_test_dataset.to_csv('y_test_dataset.csv')
...
if __name__ == '__main__':
  ...
  gait_data = gait_data.reshape(-1,125*3,1)
